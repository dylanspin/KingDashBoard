...
<style>
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    ...
                    <form 
                        id="commentForm" 
                        action="{{url('devices/edit/'.$locationDevices->id)}}" 
                        method="POST" 
                        ...
                        <input 
                            type="hidden" 
                            name="device_live_id" 
                            value="{{$locationDevices->live_id}}"/>
                        <input 
                            type="hidden" 
                            class="device_id_hidden" 
                            value="{{$locationDevices->id}}"/>
                        <div id="rootwizard">
                            ...
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab1">
                                    ...
                                    <div class="form-group {{ $errors->first('device_type', 'has-error') }}">
                                        ...
                                        <div class="col-sm-10 text-left">
                                            <input type="hidden" name="device_type" value="{{$locationDevices->available_device_id}}">
                                            @foreach ($deviceTypes as $deviceType)
                                            @if( $locationDevices->available_device_id == $deviceType->id) 
                                            <label class="col-sm-12 pt-7 font-bold pl-0">{{$deviceType->name}}</label>
                                            @endif
                                            @endforeach
                                        </div>
                                    ...
                                    <div class="form-group related_od_con {{ $errors->first('related_od', 'has-error') }}">
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control select2" multiple="" title="Select" name="related_od[]">
                                                ...
                                                <option 
                                                    value="{{$device->id}}"
                                                    @if(in_array($device->id,$device_ods))
                                                    selected="selected"
                                                    @endif
                                                    >{{$device->device_name}}
                                            </option>
                                ...
                                <div class="form-group {{ $errors->first('device_direction', 'has-error') }}">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control " title="Select Device Direction..." name="device_direction">
                                            <option value="">@lang('devices.select')</option>
                                            <option 
                                                value="bi-directional"
                                                @if(old('device_direction', $locationDevices->device_direction) == 'bi-directional') selected="selected" @endif >@lang('devices.bi_directional')
                                        </option>
                                        <option 
                                            value="in"
                                            @if(old('device_direction', $locationDevices->device_direction) == 'in') selected="selected" @endif >@lang('devices.in')
                                    </option>
                                    <option 
                                        value="out"
                                        @if(old('device_direction', $locationDevices->device_direction) == 'out') selected="selected" @endif >@lang('devices.out')
                                </option>
                            </select>
                        </div>
                    ...
                    <div class="form-group related_ticket_readers_con {{ $errors->first('related_ticket_readers', 'has-error') }}">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control"  title="Select" name="related_ticket_readers">
                                ...
                                <option 
                                    value="{{$device->id}}"
                                    @if(in_array($device->id,$device_ticket_readers))
                                    selected="selected"
                                    @endif
                                    >{{$device->device_name}}
    ...
    <div class="form-group related_device_con {{ $errors->first('related_od', 'has-error') }}">
        <label for="related_device" class="col-sm-2 control-label">@lang('devices.relate_device') *</label>
        <div class="col-sm-10">
            <select class="form-control select2" multiple="" title="Select" name="related_device[]">
                ...
                <option 
                    value="{{$device->id}}"
                    @if(in_array($device->id,$od_devices)) 
                    selected="selected"
                    @endif
                    >{{$device->device_name}}
            </option>
...
<div class="form-group {{ $errors->first('device_ip', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <input 
            ...
            class="form-control required" 
            value="{!! old('device_ip', $locationDevices->device_ip) !!}"/>
    ...
    <div class="form-group {{ $errors->first('anti_passback', 'has-error') }}">
        ...
        <div class="col-sm-10">
            <select class="form-control anti_passback" title="Select Anti Passback..." name="anti_passback">
                <option 
                    value="1"
                    @if(old('anti_passback', $locationDevices->anti_passback) == 1) selected="selected" @endif >@lang('devices.yes')
            </option>
            <option 
                value="0"
                @if(old('anti_passback', $locationDevices->anti_passback) == 0) selected="selected" @endif >@lang('devices.no')
        </option>
    </select>
</div>
<span class="help-block">{{ $errors->first('anti_passback', ':message') }}</span>
</div>
<div class="form-group time_passback_con {{old('anti_passback', $locationDevices->anti_passback) == 0 ? 'hidden' : ''}} {{ $errors->first('time_passback', 'has-error') }}">
...
<div class="form-group has_gate_con {{ $errors->first('has_gate', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <select 
            ...
            <option 
                value="0"
                @if(old('has_gate', $locationDevices->has_gate) == 0) selected="selected" @endif >@lang('devices.no')
        </option>
        <option 
            value="1"
            @if(old('has_gate', $locationDevices->has_gate) == 1) selected="selected" @endif >@lang('devices.yes')
    </option>
</select>
</div>
<span class="help-block">{{ $errors->first('has_gate', ':message') }}</span>
</div>
<div class="form-group barrier_close_time_con {{old('has_gate', $locationDevices->has_gate) == 1 ? '' : 'hidden'}} {{ $errors->first('barrier_close_time', 'has-error') }}">
...
<div class="form-group {{ $errors->first('enable_log', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <select class="form-control" title="Select Enable Log" name="enable_log">
            <option 
                value="1"
                @if(old('enable_log', $locationDevices->enable_log) == 1) selected="selected" @endif >@lang('devices.yes')
        </option>
        <option 
            value="0"
            @if(old('enable_log', $locationDevices->enable_log) == 0) selected="selected" @endif >@lang('devices.no')
    </option>
</select>
</div>
...
<div class="form-group {{ $errors->first('enable_idle_screen', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <select 
            ...
            <option 
                value="0"
                @if(old('enable_idle_screen', $locationDevices->enable_idle_screen) == 0) selected="selected" @endif >@lang('devices.no')
        </option>
        <option 
            value="1"
            @if(old('enable_idle_screen', $locationDevices->enable_idle_screen) == 1) selected="selected" @endif >@lang('devices.yes')
    </option>
</select>
</div>
...
<div class="form-group {{ $errors->first('qr_code_type', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <select class="form-control" title="Select QR Code Type" name="qr_code_type">
            <option 
                value="csv"
                @if(old('qr_code_type', $locationDevices->qr_code_type) == 'csv') selected="selected" @endif >@lang('devices.csv')
        </option>
        <option 
            value="xml"
            @if(old('qr_code_type', $locationDevices->qr_code_type) == 'xml') selected="selected" @endif >@lang('devices.xml')
    </option>
    <option 
        value="json"
        @if(old('qr_code_type', $locationDevices->qr_code_type) == 'json') selected="selected" @endif >@lang('devices.json')
</option>
</select>
</div>
<span class="help-block">{{ $errors->first('qr_code_type', ':message') }}</span>
</div>
<div class="form-group focus_away_con {{ $errors->first('focus_away', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <select class="form-control" title="Select Focus Away" name="focus_away">
            <option 
                value="1"
                @if(old('focus_away', $locationDevices->focus_away) == 1) selected="selected" @endif >@lang('devices.yes')
        </option>
        <option 
            value="0"
            @if(old('focus_away', $locationDevices->focus_away) == 0) selected="selected" @endif >@lang('devices.no')
    </option>
</select>
</div>
<span class="help-block">{{ $errors->first('focus_away', ':message') }}</span>
</div>
<div class="form-group opacity_input_con  {{ $errors->first('opacity_input', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <select class="form-control" title="Select Opacity Input" name="opacity_input">
            <option 
                value="1"
                @if(old('opacity_input', $locationDevices->opacity_input) == 1) selected="selected" @endif >@lang('devices.yes')
        </option>
        <option 
            value="0"
            @if(old('opacity_input', $locationDevices->opacity_input) == 0) selected="selected" @endif >@lang('devices.no')
    </option>
</select>
</div>
<span class="help-block">{{ $errors->first('opacity_input', ':message') }}</span>
</div>
<div class="form-group od_enabled_con  {{ $errors->first('od_enabled', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <select class="form-control" title="Select OD Enabled" name="od_enabled">
            <option 
                value="0"
                @if(old('od_enabled',$locationDevices->od_enabled) == 0) selected="selected" @endif >@lang('devices.no')
        </option>
        <option 
            value="1"
            @if(old('od_enabled',$locationDevices->od_enabled) == 1) selected="selected" @endif >@lang('devices.yes')
    </option>
    ...
    <label for="has_sdl" class="col-sm-2 control-label">@lang('devices.has_sdl') *</label>
        <div class="col-sm-10">
            <select class="form-control has_sdl" title="Select SDL" name="has_sdl">
                <option 
                    value="1"
                    @if(old('has_sdl', $locationDevices->has_sdl) == 1) selected="selected" @endif >@lang('devices.yes')
            </option>
            <option 
                value="0"
                @if(old('has_sdl', $locationDevices->has_sdl) == 0) selected="selected" @endif >@lang('devices.no')
            </option>
        </select>
    </div>
    <span class="help-block">{{ $errors->first('has_sdl', ':message') }}</span>
</div>
<div class="form-group gate_close_transaction_enabled_con {{ $errors->first('gate_close_transaction_enabled', 'has-error') }}">
    <label for="gate_close_transaction_enabled" class="col-sm-2 control-label">@lang('devices.gate_close_transaction_enabled') *</label>
        <div class="col-sm-10">
            <select class="form-control gate_close_transaction_enabled" title="Enable Register Transaction on Gate Close" name="gate_close_transaction_enabled">
                <option 
                    value="1"
                    @if(old('gate_close_transaction_enabled', $locationDevices->gate_close_transaction_enabled) == 1) selected="selected" @endif >@lang('devices.yes')
            </option>
            <option 
                value="0"
                @if(old('gate_close_transaction_enabled', $locationDevices->gate_close_transaction_enabled) == 0) selected="selected" @endif >@lang('devices.no')
            </option>
        </select>
    </div>
...
<div class="form-group has_pdl_con {{ $errors->first('has_pdl', 'has-error') }}">
    <label for="has_pdl" class="col-sm-2 control-label">@lang('devices.has_pdl') *</label>
        <div class="col-sm-10">
            <select class="form-control has_pdl" title="Enable PDL" name="has_pdl">
                <option 
                    value="1"
                    @if(old('has_pdl', $locationDevices->has_pdl) == 1) selected="selected" @endif >@lang('devices.yes')
            </option>
            <option 
                value="0"
                @if(old('has_pdl', $locationDevices->has_pdl) == 0) selected="selected" @endif >@lang('devices.no')
            </option>
        </select>
    </div>
    <span class="help-block">{{ $errors->first('has_pdl', ':message') }}</span>
</div>
<div class="form-group plate_correction_enabled_con {{ $errors->first('plate_correction_enabled', 'has-error') }}">
    <label for="plate_correction_enabled" class="col-sm-2 control-label">@lang('devices.plate_correction_enabled') *</label>
        <div class="col-sm-10">
            <select class="form-control plate_correction_enabled" title="Enable Plate Correction" name="plate_correction_enabled">
                <option 
                    value="1"
                    @if(old('plate_correction_enabled', $locationDevices->plate_correction_enabled) == 1) selected="selected" @endif >@lang('devices.yes')
            </option>
            <option 
                value="0"
                @if(old('plate_correction_enabled', $locationDevices->plate_correction_enabled) == 0) selected="selected" @endif >@lang('devices.no')
            </option>
        </select>
    </div>
...
<div class="form-group confidence_level_con  {{ $errors->first('confidence_level', 'has-error') }}">
...
<div class="form-group num_tries_con   {{ $errors->first('num_tries', 'has-error') }}">
...
<div class="form-group ccv_pos_ip_con  {{ $errors->first('ccv_pos_ip', 'has-error') }}">
...
<div class="form-group ccv_pos_port_con {{ $errors->first('ccv_pos_port', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <input 
            ...
            class="form-control ccv_pos_port" 
        ...
        {!! $errors->first('ccv_pos_port', '<span class="help-block">:message</span>') !!}
                                ...
                                <div class="form-group has_enable_person_ticket_con {{ $errors->first('has_enable_person_ticket', 'has-error') }}">
                                ...
                                <div class="form-group has_enable_parking_ticket_con {{ $errors->first('has_enable_parking_ticket', 'has-error') }}">
                                    <div class="col-sm-10 col-sm-offset-2">
                                        <div class="checkbox checkbox-primary pull-left p-t-0">
                                            <input 
                                                type="checkbox" 
...
<div class="form-group message_text_size_con {{ $errors->first('message_text_size', 'has-error') }}">
...
<div class="form-group time_text_size_con {{ $errors->first('time_text_size', 'has-error') }}">
...
<div class="form-group date_text_size_con {{ $errors->first('date_text_size', 'has-error') }}">
...
<div class="form-group bottom_tray_text_size_con {{ $errors->first('bottom_tray_text_size', 'has-error') }}">
...
<div class="form-group popup_time_con {{ $errors->first('popup_time', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <input 
            ...
            value="{!! old('popup_time', $locationDevices->popup_time) !!}"
            required=""/>
...
<div class="form-group {{ $errors->first('has_always_access', 'has-error') }}">
    ...
    <div class="col-sm-10">
        <select class="form-control has_always_access" name="has_always_access">
            <option 
                value="1"
                @if(old('has_always_access', $locationDevices->has_always_access) == 1) selected="selected" @endif >@lang('devices.yes')
            </option>
            <option 
                value="0"
                @if(old('has_always_access', $locationDevices->has_always_access) == 0) selected="selected" @endif >@lang('devices.no')
            </option>
        </select>
    </div>
    <span class="help-block">{{ $errors->first('has_always_access', ':message') }}</span>
</div>
                                @if(in_array($locationDevices->available_device_id, array(1, 2, 4, 6)))
                                    ...
                                    <div class="col-sm-10">
                                        <div class="fileinput fileinput-new" data-provides="fileinput">
                                            <div class="fileinput-new thumbnail"
                                                ...
                                                <img src="{{asset($locationDevices->advert_image_path)}}" alt="advert image pic">
                                                @else
                                ...
                                @endif
								@if(in_array($locationDevices->available_device_id, array(1, 2)))
                                    ...
                                    <div class="col-sm-10">
                                        <div class="fileinput fileinput-new" data-provides="fileinput">
                                            <div class="fileinput-new thumbnail"
                                                ...
                                                <img src="{{asset($locationDevices->idle_screen_image)}}" alt="Idle screen image">
                                                @else
