...
//
        ...
        $on_location_person_bookings = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) {
                        $query->whereIn('type', array(5, 6))->whereNull('vehicle_num')->where('is_tommy_online', 0)->whereNull('tommy_childeren_id');
                    }
                    )->whereNull('check_out')
                    ->count();
        $on_location_person_empty_vehicle = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) {
