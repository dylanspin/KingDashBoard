...
class DashboardController extends Controller {
     ...
     * @return type
     */
    function p_dashboard(Request $request) {
     ...
     * @return type
     */
    function get_current_week_start_end() {
     ...
     * @return type
     */
    function get_previous_week_start_end() {
     ...
     * @return type
     */
    function get_bookings($start_date, $end_date) {
     ...
     * @return type
     */
    function get_this_week_bookings() {
     ...
     * @return type
     */
    function get_previous_week_bookings() {
     ...
     * @return type
     */
    function get_this_week_revenue() {
        $revenue = 0;
        $dates = $this->get_current_week_start_end();
        $bookings = $this->get_bookings($dates->start, $dates->end);
        if ($bookings) {
     ...
     * @return type
     */
    function get_today_booking_details($type = NULL) {
        $total = array();
        $arrived = array();
        $expecting = array();
        $at_location = array();
...
//                    }
     ...
     * @return type
     */
    function get_camera_enabled_device() {
        $response = array();
     ...
     * @return type
     */
    function get_ticket_enabled_device() {
        $response = array();
     ...
     * @return type
     */
    function get_last_5_transactions_1($limit = NULL, $das_p = FALSE) {
        $response = array();
        ...
        if ($attendant_transaction->count() > 0) {
            foreach ($attendant_transaction as $transaction) {
                ...
                if ($booking_details->type == 0) {
                    $data['type'] = 'N/A';
                } else if ($booking_details->type == 1) {
                    $data['type'] = 'Send Ticket';
                } else if ($booking_details->type == 2) {
                    $data['type'] = 'White List';
                } else if ($booking_details->type == 3) {
                    $data['type'] = 'User List';
                } else if ($booking_details->type == 4) {
                    $data['type'] = 'Customer';
                } else if ($booking_details->type == 5) {
                    $data['type'] = 'BarCode';
                } else if ($booking_details->type == 6 || $booking_details->type == 7) {
    ...
    function get_last_5_transactions($limit = NULL, $das_p = FALSE) {
        if (!$das_p) {
...
//                }
     ...
     * @return type
     */
    function open_gate(Request $request) {
        try {
...
//                $request->session()->put('open_gate_reason', $open_gate_reason);
            ...
            } else if ($location_devices->device_direction == 'out') {
                $booking_valid = FALSE;
                $booking = \App\Bookings::where('vehicle_num', $open_gate_vehcile_num)->orderBy('checkin_time', 'DESC')->first();
                if ($booking) {
...
//                                $transaction_images->save();
     ...
     * @return type
     */
    function no_entrance_transaction(Request $request) {
        try {
            $location = new \App\Http\Controllers\Settings\LocationSettings();
            $device_id = $request->device_id;
            $key = strtotime($location->get_location()->created_at) . '-' . $device_id;
...
//            ]);
     ...
     * @return type
     */
    function close_gate(Request $request) {
        $response = 0;
    ...
    public function at_location_vehicles($limit = NULL) {
        $at_location = array();
        $type = [1, 2, 3, 4, 5];
        $transactions = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) {
     ...
     * @return type
     */
    function edit_vehicle_num(Request $request) {
                ...
                <table class="table table-bordered update_vehicle_form">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-left text-info" style="padding:10px 30px;"><?php echo $booking_Details->confidence; ?></td>
                        </tr>
                        <?php if (!$booking_Details->low_confidence) { ?>
                            <tr>
                                <th style="padding:10px">Vehicle</th>
                                <td class="text-left text-info" style="padding:10px 30px;"><?php echo $booking_Details->vehicle_num; ?></td>
                            </tr>
                        <?php } else { ?>
...
//        ));
     ...
     * @return type
     */
    function edit_device_vehicle_num(Request $request) {
     ...
     * @return type
     */
    function low_confience_details($id) {
     ...
     * @return type
     */
    function check_vehicle_number($num1, $num2, $char_to_match) {
...
//                $content = $quantity . ' Persons - ' . date('d/m/Y H:i', strtotime($person_payment_transaction->updated_at)) . ' - ' . $status;
    ...
    public function get_widget1_details($das_p = FALSE, $recall = FALSE) {
        ...
        if ($das_p) {
            $type = 6;
            $is_online = 1;
            $on_location_bookings = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) {
                        $query->whereIn('type', array(5, 6))->whereNull('vehicle_num')->where('is_tommy_online', 0)->whereNull('tommy_childeren_id');
                    }
                    )->whereNull('check_out')
                    ->count();
            $on_location_empty_vehicle = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) {
                        $query->whereIn('type', array(5, 6))->where('vehicle_num', '')->where('is_tommy_online', 0)->whereNull('tommy_childeren_id');
                    }
                    )->whereNull('check_out')
                    ->count();
            $on_location = $on_location_bookings + $on_location_empty_vehicle;

            $arrivals = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) use ($type) {
            ...
            $available_spots = $total_spots_person - ($not_checked_in_bookings + $on_location);
            $total_spots = $total_spots_person;
            $data_array = (object) array(
                        'on_location' => $on_location,
                        'arrivals' => $arrivals,
                        'expected_bookings' => $expected_bookings,
                        'arrival_d' => $not_checked_in_bookings,
            );
            ...
            $type = [1, 2, 3, 4, 5];
            $on_location = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) use ($type) {
            ...
            $checked_in_known_bookings_parking = \App\Bookings::whereHas('attendant_transactions', function ($query) {
                                $query->whereNull('check_out');
                            })
                            ->where('type', 4)->where('checkout_time', '=', date('Y-m-d 23:59:59'))->pluck('id')->all();

            $not_checked_in_bookings = \App\Bookings::whereNotIn('id', $checked_in_known_bookings_parking)->where('checkout_time', '=', date('Y-m-d 23:59:59'))->where('checkin_time', '<=', date('Y-m-d H:i:s'))->where('type', 4)->count();

            $expected_bookings = count($checked_in_known_bookings_parking) + $not_checked_in_bookings;
//            $total_bookings = $on_location + $arrivals;
            $total_bookings = $expected_bookings;
            $data_array = (object) array(
                        'on_location' => $on_location,
                        'arrivals' => $arrivals,
                        'expected_bookings' => $expected_bookings,
            ...
            $available_spots = $total_spots - $on_location - $not_checked_in_bookings;
        }
        return (object) array(
    ...
    public function get_widget2_details($das_p = FALSE) {
        ...
        if ($das_p) {
            ...
            $on_location_empty_vehicle = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) {
                    ...
                    ->count();
            $on_location = $on_location_bookings + $on_location_empty_vehicle;
            $expected_bookings_other = \App\Bookings::whereDate('checkin_time', '=', Carbon::today())
                    ->where('checkout_time', date('Y-m-d 23:59:59'))
                    ->where('type', $type)
                    ->where('is_tommy_online', 0)->whereNull('tommy_childeren_id')
                    ->count();

            $expected_bookings_yearly = \App\Bookings::where('checkout_time', date('Y-12-31 23:59:59'))
                    ->where('type', $type)
                    ->count();

            $expected = $expected_bookings_other + $expected_bookings_yearly;
            $at_location_bookings_barcode = \App\AttendantTransactions::whereHas('attendants.bookings', function ($query) {
                        $query->where('type', 5)->whereNull('vehicle_num');
                    }
                    )->whereNull('check_out')
                    ->count();
            $total = $expected + $at_location_bookings_barcode;
        } else {
            ...
            if ($booking_details) {
                ...
                $expected_bookings_other = \App\Bookings::where('checkout_time', '>=', date('Y-m-d H:i:s'))
                        ->whereDate('checkin_time', Carbon::today())
                        ->whereIn('type', [1, 4])
                        ->whereColumn('checkin_time', '<>', 'created_at')
                        ->count();
                $expected_bookings_yearly = \App\Bookings::where('checkout_time', date('Y-12-31 23:59:59'))
                        ->whereIn('type', [1, 4])
                        ->count();
    ...
    function open_gate_person(Request $request) {
        $response = 0;
        if (isset($request->device_id)) {
    ...
    function close_gate_person(Request $request) {
        $response = 0;
        if (isset($request->device_id)) {
            $location_device = \App\LocationDevices::find($request->device_id);
            if ($location_device->barrier_status < 1) {
...
//            ]);
                    ...
                    if ($request->search_type != 'first_name' && $request->search_type != 'vehicle') {
                        $attendant_transaction = $attendant_transaction->whereHas('open_gate_manual_transaction', function ($query) use ($request) {
                                    if ($request->search_type == 'open_gate_manual') {
                                        $query->where('type', 'MO');
                                    } else if ($request->search_type == 'open_gate_always') {
                                        $query->where('type', 'AA');
                                    }
                                ...
                                })->with(['open_gate_manual_transaction' =>
    ...
    function change_gate_barrier_status(Request $request) {
        $response = 0;
        $message = 'Please wait process is in progress';
        if (isset($request->device_id)) {
            if ($request->status == 1) {
    ...
    function un_lock_gate($device_id) {
        $response = 0;
        if (isset($device_id)) {
    ...
    function locked_open_gate($device_id) {
        $response = 0;
        if (isset($device_id)) {
            $device = \App\LocationDevices::find($device_id);
            if ($device) {
    ...
    function locked_close_gate($device_id) {
        $response = 0;
        if (isset($device_id)) {
            $device = \App\LocationDevices::find($device_id);
            if ($device) {
