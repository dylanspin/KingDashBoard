...
//            exit;
    ...
    public function getPayments(Request $request) {
        ...
        $filter_valid_dates = '';
        $bookingPayments = \App\BookingPaymentsView::sortable()->orderBy('check_in', 'DESC');
...
//                'filter_time_type', 
    ...
    public function getAttendants() {
        ...
        return view('location.attendants', compact('attendantsInfo'));
    }
    ...
    public function update(Request $request)
    {
        try {
            ...
            $location_spots = $dashboardController->get_widget1_details();
            //$data['on_location_spots'] = $location_spots->on_location ? $location_spots->on_location : 0;
            try {
				...
				$bgName = "";
                if ($logoFile) {
                    $logo_image_path = $logoFile->getPathname();
                    $logoName = $logoFile->getClientOriginalName();
                }
                $locFile = $request->file('loc_file');
				$loc_image_path = "";
				
                if ($locFile) {
                    $loc_image_path = $locFile->getPathname();
                    $locName = $locFile->getClientOriginalName();
					
                }
                $bgFile = $request->file('bg_path');
				$loc_bg_path = "";
                if ($bgFile) {
                    $loc_bg_path = $bgFile->getPathname();
                    $bgName = $bgFile->getClientOriginalName();
                }
                $http = new Client();
                $response = $http->request('POST', env('API_BASE_URL') . '/api/update-location-data', [
                    'multipart' => [
                        [
                            'name' => 'token',
                            'contents' => $Key
                        ...
                        [
                            'name' => 'logo_file',
                            'contents' => !empty($logo_image_path) ? file_get_contents($logo_image_path) : null,
                            'filename' => $logoName
                        ],
                        [
                            'name' => 'loc_file',
                            'contents' => !empty($loc_image_path) ? file_get_contents($loc_image_path) : null,
                            'filename' => $locName
                        ],
                        [
                            'name' => 'bg_path',
                            'contents' => !empty($loc_bg_path) ? file_get_contents($loc_bg_path) : null,
                            'filename' => $bgName
                        ],
                        [
                            'name' => 'data',
            ...
            if (array_key_exists('is_bikes', $data)) {
            ...
            if (array_key_exists('is_doors', $data)) {
            ...
            if (array_key_exists('is_ev_charger_available', $data)) {
			...
			if (array_key_exists('map_enable', $data)) {
                $location->map_enable = $data['map_enable'];
            }
            if (array_key_exists('reply_to_name', $data)) {
                $location->reply_to_name = $data['reply_to_name'];
            }
            if (array_key_exists('reply_to_email', $data)) {
                $location->reply_to_email = $data['reply_to_email'];
            }
            if (array_key_exists('location_phone_num', $data)) {
                $location->location_phone_num = $data['location_phone_num'];
            }
            if (array_key_exists('real_time_payments', $data)) {
                $location->real_time_payments = $data['real_time_payments'];
            }
            if ($file = $request->file('logo_file')) {
                $destinationPath = public_path('/uploads/location');
                $safeNamee = Str::random(4).time() . '.' . $file->getClientOriginalExtension();
                $img = Image::make($file->getPathName());
                $img->resize(null, 237, function ($constraint) {
                    $constraint->aspectRatio();
                    $constraint->upsize();
                });
                $img->save($destinationPath . '/' . $safeNamee);
                //delete old pic if exists
                if (File::exists(public_path($location->logo_path))) {
                    File::delete(public_path($location->logo_path));
                }
                //save new file path into db
                $location->logo_path = '/uploads/location/' . $safeNamee;
            }
            if ($file = $request->file('loc_file')) {
                $extension = $file->extension() ?: 'png';
                $destinationPath = public_path('/uploads/location');
                $safeName =  Str::random(4).time() . '.' . $extension;
                $file->move($destinationPath, $safeName);
                //delete old pic if exists
                if (File::exists(public_path($location->loc_path))) {
                    File::delete(public_path($location->loc_path));
                }
                //save new file path into db
                $location->loc_path = '/uploads/location/' . $safeName;
            }
            if ($file = $request->file('bg_path')) {
                $extension = $file->extension() ?: 'png';
                $destinationPath = public_path('/uploads/location');
                $safeName =  Str::random(4) . time() . '.' . $extension;
                $file->move($destinationPath, $safeName);
                //delete old pic if exists
                if (File::exists(public_path($location->bg_path))) {
                    File::delete(public_path($location->bg_path));
                }
                //save new file path into db
                $location->bg_path = '/uploads/location/' . $safeName;
            }
            $location->save();
            ...
            if (array_key_exists('today_hours', $data)) {
                $today_hours = LocationTodayTime::first();
                if (!$today_hours) {
                    $today_hours = new LocationTodayTime();
                    $today_hours->opening_time = $data['today_opening_time'];
                    $today_hours->closing_time = $data['today_closing_time'];
                    $today_hours->date = date("Y-m-d");
                    $today_hours->save();
                } else {
                    $today_hours->opening_time = $data['today_opening_time'];
                    $today_hours->closing_time = $data['today_closing_time'];
