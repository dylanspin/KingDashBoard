...
class PaymentTerminal extends Controller {
    ...
    public function get_booking_time_payment($vehicle_booking) {
        try {
			$total_checkin_checkout_difference = $this->get_difference_time(date('Y-m-d H:i', strtotime($vehicle_booking->checkin_time)), date('Y-m-d H:i',strtotime($vehicle_booking->checkout_time)), 3);
            if ($vehicle_booking->checkout_time != NULL) {
				...
				else if($vehicle_booking->booking_payments != null && $total_checkin_checkout_difference != 15){
    ...
    public function get_booking_price($vehicle_booking) {
		...
		$total_checkin_checkout_difference = $this->get_difference_time(date('Y-m-d H:i', strtotime($vehicle_booking->checkin_time)), date('Y-m-d H:i',strtotime($vehicle_booking->checkout_time)), 3);
        if ($vehicle_booking->checkout_time != NULL) {
            if ($vehicle_booking->checkout_time > date('Y-m-d H:i') && $total_checkin_checkout_difference != 15) {
                return array(
                    'status' => 0,
        ...
        if (date('Y-m-d', strtotime($booking_time)) == date('Y-m-d')) {
            ...
            $hourly_price = (int)$at_location_minutes * (double)$this->price_per_minute;
        ...
        try {
            ...
            $checked_in_bookings = \App\Bookings::whereHas('attendant_transactions', function ($query) {
                    ...
                    ->where(function($query) {
						...
						$query->orWhere('checkin_time', '>', \DB::raw('DATE_SUB(NOW(), INTERVAL 15 MINUTE)'));
                    })
                    ->orderBy('created_at', 'desc')
                    ->take(5)
                    ->get();
