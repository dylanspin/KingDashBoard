...
class VerifyVehicle extends Controller {
    ...
    public function check_is_booking_missed($vehicle_numbers, $booking = FALSE, $live_booking_id = FALSE, $live_booking_payment_id = FALSE) {
        try {
            $device_booking = \App\DeviceBookings::where([
                                    ['vehicle_num', $vehicle_numbers]
                                ])
                    ->whereIn('device_id', [60, 25])
            ...
            if (!$booking) {
                ...
                $booking = \App\Bookings::where([
                        ...
                        ->first();
                if (!$booking) {
                    $checkout_time = date('Y-m-d H:i:s', strtotime('+15 minutes', strtotime($device_booking->created_at)));
                    $dataArray = array(
                    ...
                    if (!$live_booking_id && !$live_booking_payment_id) {
                        try {
                            ...
                            $error_log = new \App\Http\Controllers\LogController();
                            $error_log->log_create('missed_booking_function', $ex->getMessage(), $ex->getTraceAsString());
                            return $booking;
                        }
                    } else {
                        $booking = \App\Bookings::find($bookingId);
...
//                ->update(['check_out' => date('Y-m-d H:i:s')]);
    ...
    public function verify_plate_num(Request $request, $key, $id, $vehicle, $confidence, $country_code = NULL) {
        ...
        try {
            ...
            if ($valid_settings->emergency_entry_exit) {
...
//                    ]);
            ...
            if (!$confidence_status) {
                ...
                if ($confidence == 0) {
                    $temp_booking = $this->set_temporary_booking_entry($valid_settings->id, $vehicle, $confidence, $file_path, $country_code);
            ...
            if ($valid_settings->device_direction == 'in') {
                $vehicle_booking = $this->get_vehicle_booking($vehicle, 'in', $valid_settings, $confidence_status);
                if ($valid_settings->barrier_status == 3 || (!$valid_settings->has_gate && isset($valid_settings->device_ticket_reader) && $valid_settings->device_ticket_reader->barrier_status == 3)) {
                    if (strtolower($vehicle) == 'none') {
                        $vehicle = "NO PLATE";
                    }
                    if (!$vehicle_booking) {
                        $vehicle_booking = $this->create_booking($vehicle, $valid_settings->id, false);
                    }
                    $temp_booking->booking_id = $vehicle_booking->id;
                    $temp_booking->save();
...
//                    }
                ...
                if ($valid_settings->barrier_status == 3 || (!$valid_settings->has_gate && isset($valid_settings->device_ticket_reader) && $valid_settings->device_ticket_reader->barrier_status == 3)) {
                    if (!$vehicle_booking) {
                        $vehicle_booking = $this->create_booking($vehicle, $valid_settings->id, false, 8);
                    }
                    $temp_booking->booking_id = $vehicle_booking->id;
                    $temp_booking->save();
                    $user_name = $this->ticket_reader->get_user_name($vehicle_booking);
                ...
                if (!$vehicle_booking) {
                    if (!$confidence_status || $vehicle == '0') {
                        $message = $this->ticket_reader->get_error_message('goto_nearby_ticket_reader', '', $this->lang_id);
                        return array(
                            'status' => 1,
                            'access_status' => 'processing',
					...
					//Work around for avoiding confusion
                    $message = $this->ticket_reader->get_error_message('goto_nearby_payment_terminal', $vehicle, $this->lang_id);
//                    Artisan::call('command:DeniedAccess', [
//                        'device' => $id, 'message' => $message, 'vehicle_num' => $vehicle
//                    ]);
                    $this->send_denied_access_socket($valid_settings, $message, $vehicle);
...
//        ]);
    ...
    public function get_vehicle_booking($vehicle_num, $status, $valid_settings = NULL, $confidence_status = true, $match = true) {
        try {
            ...
            if ($status == 'in') {
                ...
                if ($booking_details) {
                    ...
                    //}
                } else if ($confidence_status && $vehicle_num != '0') {
                    return $this->create_booking($vehicle_num, $valid_settings->id, false);
                }
                return false;
            } elseif ($status == 'out') {
                $user_list_statuses = array(2, 3);
                $vehicle_numbers = array($vehicle_num);
//                if ($match) {
//                    if (strlen($vehicle_num) >= 4) {
//                        for ($i = 0; $i <= strlen($vehicle_num) % 4; $i++) {
//                            $vehicle_numbers[] = substr($vehicle_num, $i, 4);
//                        }
//                    }
//                }
                $userlist_booking_details = \App\Bookings::where([
            ...
            if ($status) {
                if (strtolower($vehicle) == 'none') {
                    return array(
                        'status' => 1,
                        'access_status' => 'denied',
                        'message' => "None plate is not registered",
                        'data' => FALSE,
                    );
                }
                if ($valid_settings->device_direction == 'in') {
                    ...
                    if ($vehicle_booking) {
                        ...
                        //                            );
                        //                        }
                        $this->push_booking_cloud($vehicle_booking);
                    ...
                    $message = $this->ticket_reader->get_error_message('welcome_entrance', '', $this->lang_id);
                    $booking = $this->create_booking($vehicle, $valid_settings->id, false);
                    $this->push_booking_cloud($booking);
                    $this->set_booking_entry($booking, $valid_settings->id);
                    return array(
                        'status' => 1,
                        'access_status' => 'success',
                        'message' => $message,
                    ...
                    $vehicle_booking = $this->get_vehicle_booking($vehicle, 'out', null, true, false);
                    if (!$vehicle_booking) {
                        $vehicle_booking = $this->create_booking($vehicle, $valid_settings->id, false, 8);
...
//                    }
    ...
    public function set_booking_entry($booking_details, $device_id) {
        ...
        $attendant_transaction->save();
        $this->update_transaction_table($device_id, $attendant_transaction->id, 'in', $booking_details->id);
        \Illuminate\Support\Facades\Session::forget('vehicle_image');
        \App\DeviceBookings::where('device_id', $device_id)->update(array('is_operator' => 0));
        ...
        try {
            ...
            $error_log = new \App\Http\Controllers\LogController();
            $error_log->log_create('set_booking_entry', $ex->getMessage(), $ex->getTraceAsString());
        }
        return TRUE;
    }
    ...
    public function set_booking_exit($booking_details, $device_id) {
        ...
        $this->update_transaction_table($device_id, $attendant_transaction->id, 'out', $booking_details->id);
        \Illuminate\Support\Facades\Session::forget('vehicle_image');
        \App\DeviceBookings::where('device_id', $device_id)->update(array('is_operator' => 0));
        $this->update_booking_from_temporary_booking($device_id, $booking_details->id);
        $attendants = array();
    ...
    public function set_booking_exit_cloud_cron_job() {
        try {
            if (!$this->key) {
                $error_log = new \App\Http\Controllers\LogController();
                $error_log->log_create('import-key', 'custom: Import key not found');
            ...
            $http = new Client();
            $response = $http->post($this->url . '/api/set-bookings-checkout-location', [
                'form_params' => [
                    'token' => $Key
                ],
                'connect_timeout' => 5
            ...
            $error_log = new \App\Http\Controllers\LogController();
            $error_log->log_create('set_booking_exit_cloud_cron_job', $ex->getMessage(), $ex->getTraceAsString());
        }
        return TRUE;
    }

    public function create_booking($vehicle, $device_id, $check_in_vehicle = True, $type = 4) {
        $dataArray = array(
            'first_name' => 'Paid Vehicle',
            'vehicle_num' => $vehicle,
            'type' => $type,
    ...
    public function push_booking_cloud($booking) {
        ...
        try {
            ...
            $error_log = new \App\Http\Controllers\LogController();
            $error_log->log_create('push_booking_cloud', $ex->getMessage(), $ex->getTraceAsString());
        }
    }
    ...
    public function update_booking_from_temporary_booking($device_id, $booking_id) {
        ...
        $low_confidence = 0;
        $related_plate_reader_id = !$related_plate_reader ? $device_id : $related_plate_reader->device_id;
        $device_details = \App\LocationDevices::find($related_plate_reader_id);
        if (!$device_details) {
            return FALSE;
        }
        $booking = \App\Bookings::find($booking_id);
        $temporary_booking = \App\DeviceBookings::where('device_id', $related_plate_reader_id)->where('booking_id', $booking->id)
                ->orderBy('created_at', 'DESC')
                ->first();
    ...
    public function update_transaction_table($device_id, $attendant_id, $status, $booking_id = 0) {
        $related_plate_reader = \App\DeviceTicketReaders::where([
                    ['ticket_reader_id', $device_id]
                ])->first();
        ...
        if ($booking_id > 0) {
            $temporary_booking = \App\DeviceBookings::where('device_id', $related_plate_reader_id)->where('booking_id', $booking_id)
                    ->orderBy('created_at', 'DESC')
                    ->first();
            if (!$temporary_booking) {
                return FALSE;
