...
class VerifyBookings extends Controller {
    ...
    public function verify_booking(Request $request, $key, $id, $type, $val, $ticket_type = FALSE) {
        try {
            ...
            if ($valid_settings->anti_passback) {
                $valid_passback = $this->is_valid_antipassback($booking_details, $id);
                if (!$valid_passback['status']) {
                    $message = $this->get_error_message($valid_passback['message_code'], $user_name);
                    return array(
                        'status' => 1,
                        'access_status' => 'denied',
                        'od_sent' => $settings->send_message_od($id, $message, 'rejected'),
                ...
                if ($booking_details->type == 3) {
                    ...
                    if ($userlist_user['status'] == 2) {
                        ...
                        if (!$whitelist_timings['status']) {
                            $message = $this->get_error_message($whitelist_timings['status'], $user_name, $userlist_user['data']->language_id);
                            return array(
                                'status' => 1,
                                'access_status' => 'denied',
                                'od_sent' => $settings->send_message_od($id, $message, 'rejected'),
            ...
            if (!$ticket_type || $ticket_type == 'whitelist') {
                if ($booking_details->type == 2) {
                    ...
                    if ($whitelist_user) {
                        $whitelist_timings = $this->valid_whitelist_timings($booking_details);
                        if (!$whitelist_timings['status']) {
                            $message = $this->get_error_message($whitelist_timings['status'], $user_name);
                            return array(
                                'status' => 1,
                                'access_status' => 'denied',
                                'od_sent' => $settings->send_message_od($id, $message, 'rejected'),
    ...
    public function valid_whitelist_timings($booking) {
...
//                        );
    ...
    public function is_valid_group_device($group_id) {
        if (!$this->device_id) {
            return FALSE;
        }
        if ($group_id == NULL) {
