...
//        $bookingPayments = $bookingPayments->groupBy('email');
    ...
    public function download($id) {
        try {
            ...
            else if (date('Y-m-d H:i:s', strtotime($booking->checkin_time)) > date('Y-m-d 00:00:00') && date('Y-m-d H:i:s', strtotime($booking->checkout_time)) > date('Y-m-d 23:59:59')) {
                if ($booking->type == 6 || $booking->type == 10) {
                    if ($current_locale == 'en') {
                        $title = 'Day ticket person';
                    } else {
    ...
    public function delete(Request $request) {
        try {
            $current_locale = \App::getLocale();
            $booking = Bookings::find($request->hidden_booking_id);
            if ($booking->type==6) {
				$booking->delete();
    ...
    public function generate_booking_pdf($booking_id, $title, $ticket_type, $ticket_type_name, $barcode_number) {
        try {
			...
			if ($current_locale == 'en') {
                $pdf_title = 'Reserving ' . $products->title;
            } else {
                $pdf_title = 'Reservering ' . $products->title_nl;
            }
            $type = 0;
            $how_to_use = 0;
			...
			$map_enable = $location_options->map_enable;
            $fie_name = $title . rand() . '.pdf';
			
			if ($current_locale == 'en') {
                $title = $products->title;
            } else {
                $title = $products->title_nl;
            }
            return \PDF::loadView('day.pdf', compact(    
                    ...
                    'longitude', 
                    'dob',
					'map_enable'))->setWarnings(false)->download($fie_name);
        } 
        catch (\Exception $ex) {
            return FALSE;
        }
    ...
    public function resend($id) {
        try {
            ...
            if (date('Y-m-d H:i:s', strtotime($booking->checkin_time)) == date('Y-m-d 00:00:00') && date('Y-m-d H:i:s', strtotime($booking->checkout_time)) == date('Y-m-d 23:59:59')) {
				...
				$data['check_in'] = $booking->checkin_time;
				$data['check_out'] = $booking->checkout_time;
    ...
    public function update(Request $request){
        ...
        $check_in = date('Y-m-d 00:00:00', strtotime($request->arrival_time));
        $check_out = date('Y-m-d 23:59:59', strtotime($request->departure_time));
