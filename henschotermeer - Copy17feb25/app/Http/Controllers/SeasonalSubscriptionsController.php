...
class SeasonalSubscriptionsController extends Controller {

    public $controller = 'App\Http\Controllers\SeasonalSubscriptionsController';
    ...
    public function person_subscriptions(Request $request) {
        ...
        //$bookingPayments = $bookingPayments->groupBy(['email', 'first_name', 'dob']);
        $bookingPayments = $bookingPayments->paginate(25);
		//print_r($bookingPayments);die();
        $totalAmount = \App\BookingPaymentsView::where('type', 6)
                ...
                ->get()
                ->sum('amount');
        return view('seasonal.person_payments', compact('bookingPayments', 'totalAmount', 'search_type', 'search_val'));
    }
    ...
    public function parking_subscriptions(Request $request) {
        ...
        $totalAmount = \App\BookingPaymentsView::where('type', 4)
                ->where('check_out', date('Y-12-31 23:59:59'))
                ->sum('amount');
        return view('seasonal.parking_payments', compact('bookingPayments', 'totalAmount', 'search_type', 'search_val'));
    }
    ...
    public function download($id) {
        try {
            ...
            if (date('Y-m-d H:i:s', strtotime($booking->checkin_time)) < date('Y-m-d H:i:s') && date('Y-m-d H:i:s', strtotime($booking->checkout_time)) == date('Y-12-31 23:59:59')) {
                if ($booking->type == 6 || $booking->type == 10) {
                    if ($current_locale == 'en') {
                        $title = 'Seasonal subscription person';
                    } else {
                        $title = 'Seizoenabonnement personen';
                    }
                    $ticket_type = 'year_ticket_person';
                    $ticket_type_name = 'person_ticket';
                    $pdf = $this->generate_booking_pdf($booking->id, $title, $ticket_type, $ticket_type_name, $barcode_number);
                } else {
                    if ($current_locale == 'en') {
                        $title = 'Annual parking subscription';
                    } else {
                        $title = 'Jaarabonnement parkeren';
                    }
                    $ticket_type = 'year_ticket_vehicle';
                ...
                Session::flash('heading', 'Error!');
                Session::flash('message', 'This is not yearly booking.');
                Session::flash('icon', 'error');
                return redirect()->back();
            }
        } catch (\Exception $e) {
    ...
    public function generate_booking_pdf($booking_id, $title, $ticket_type, $ticket_type_name, $barcode_number) {
        try {
            ...
            return \PDF::loadView('seasonal.pdf', compact(
                                    'current_locale', 'how_to_use', 'type', 'title', 'pdf_title', 'title_message', 'user_name', 'location_title', 'checkin_time', 'checkout_time', 'location_address', 'amount', 'vehicle_number_plate', 'location_description', 'location_contact', 'parking_location_image_pdf', 'qr_code_200', 'latitude', 'longitude', 'dob'))->setWarnings(false)->download($fie_name);
        } catch (\Exception $ex) {
            return FALSE;
        }
    }
    ...
    {
        try {
            $bookingId = $request->blocked_booking_id;
            $booking = Bookings::find($bookingId);
            if ($booking != null) {

                $booking->is_blocked == 0 ? $booking->is_blocked = 1 : $booking->is_blocked = 0;
                $booking->save();
                Session::flash('heading', 'Success!');
                if ($booking->is_blocked == 1) {
                    Session::flash('message', __('payments.block_vehicle'));
                } else {
                    Session::flash('message', __('payments.unblock_vehicle'));
                }
                Session::flash('icon', 'success');
                return redirect('seasonal/parking');
            } else {
                Session::flash('heading', 'Error!');
                Session::flash('message', __('payments.not_found'));
                Session::flash('icon', 'error');
                return redirect()->back();
            }
        } catch (\Exception $e) {
    ...
    public function blockOrUnblockPerson(Request $request)
    {
        try {
            $bookingId = $request->blocked_booking_id;
            $booking = Bookings::find($bookingId);
            if ($booking != null) {
                ...
                Session::flash('heading', 'Success!');
                if ($booking->is_blocked == 1) {
                    Session::flash('message', __('payments.block_person'));
                } else {
                    Session::flash('message', __('payments.unblock_person'));
                }
                Session::flash('icon', 'success');
                return redirect('seasonal/person');
            } else {
                Session::flash('heading', 'Error!');
                Session::flash('message', __('payments.not_found'));
                Session::flash('icon', 'error');
    ...
    public function resend($id) {
        try {
            ...
            if (date('Y-m-d H:i:s', strtotime($booking->checkin_time)) < date('Y-m-d H:i:s') && date('Y-m-d H:i:s', strtotime($booking->checkout_time)) == date('Y-12-31 23:59:59')) {
                $ticket_type = 'year_ticket_vehicle';
                $title = 'Year Ticket Vehicle';
                if ($booking->type == 6 || $booking->type == 10) {
                    $ticket_type = 'year_ticket_person';
                    $title = 'Year Ticket Person';
                }
                $data = array();
                ...
                try {
                    ...
                    $response = $http->post(env('API_BASE_URL').'/api/send-seasonal-ticket', [
                        'form_params' => [
                            'token' => $Key,
                            'data' => $data
                        ],
    ...
    public function update(Request $request){
		if(empty($request->booking_id)){
			Session::flash('heading', 'Error!');
			Session::flash('message', 'Something went wrong. Please contact administrator');
			Session::flash('icon', 'error');
			return redirect()->back();
		}
        ...
        if($request->date_of_birth != null){
            ...
            $booking->tommy_children_dob=date('Y-m-d', strtotime($request->date_of_birth));
        }
        if($request->vehicle != null){
            $booking->vehicle_num=$request->vehicle;
        }
    ...
    public function delete(Request $request) {
        ...
        try {
            ...
            Session::flash('message', $e->getMessage());
            Session::flash('icon', 'error');
            return redirect()->back('seasonal/person');
        }
    }
