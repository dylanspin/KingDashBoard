...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="white-box">
                <h3 class="box-title pull-left">@lang('sidebar.day') @lang('sidebar.seasonal_parking')</h3>
<!--                <p class="pull-right">
                ...
                <div class="row">
                    <div class="col-md-12 text-right">
                        <form method="post" action="{{url('/day/parking')}}" class="col-md-6 text-right custom-search-form">
                            @csrf
                            ...
                            <table id="listingDataTable" class="table table-striped">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{date('d/m/Y H:i', strtotime($bookingPayment->check_in)) ? date('d/m/Y H:i', strtotime($bookingPayment->check_in)) : 'N/A'}}</td>
                                        <td>{{date('d/m/Y H:i', strtotime($bookingPayment->check_out)) ? date('d/m/Y H:i', strtotime($bookingPayment->check_out)) : 'N/A'}}</td>
                                        ...
                                        <td>
                                            ...
                                            <button type="button" class="btn btn-danger blockedVehicle" data-id="{{ $bookingPayment->booking_id}}"  id="unblocked-vehicle"  onclick="unblocked_vehicle({{ $bookingPayment->booking_id}})"><i class="fa fa-lock"></i></button>
                                            @else
                                            <button type="button" class="btn btn-success unblockedVehicle" data-id="{{ $bookingPayment->booking_id}}"  id="blocked-vehicle" onclick="blocked_vehicle({{ $bookingPayment->booking_id}})"><i class="fa fa-unlock"></i></button>
                                            @endif
                                            ...
                                            <button type="button" class="btn btn-primary editParkingTicket" data-id="{{ $bookingPayment->booking_id}}"  id="edit-subscription"><i class="fa fa-edit"></i></button>
											<button type="button" class="btn btn-danger Deletebutton delete_button" 
												id="Delbutton" onclick="delete_day_ticket({{$bookingPayment->booking_id}})"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                        </td>
                                    </tr>
...
<div id="blockedModal" class="modal fade">
  <div class="modal-dialog modal-confirm">
    <div class="modal-content">
	<form action="{{url('day/block-unblock')}}" method="POST">
	@csrf
		  <div class="modal-header flex-column">
			<div class="icon-box">
			  <i class="fa fa-exclamation"></i>
		  ...
		  <div class="modal-body">
			<p>@lang('payments.block_really_vehicle') </p>
		  </div>
		  <div class="modal-footer justify-content-center">
...
<div id="unblockedModal" class="modal fade">
  <div class="modal-dialog modal-confirm">
    <div class="modal-content">
	<form action="{{url('day/block-unblock')}}" method="POST">
	@csrf
		  ...
		  <div class="modal-body">
			<p>@lang('payments.unblock_really_vehicle') </p>
		  </div>
		  <div class="modal-footer justify-content-center">
    ...
    <div class="modal fade" id="eidtModal" tabindex="-1" role="dialog" aria-labelledby="eidtModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            ...
            <div class="modal-header">
              ...
              <h5 class="modal-title" id="eidtModalLabel">@lang('Edit')</h5>
...
function printErrorMsg (msg) {
    ...
    $.ajaxSetup({
          ...
          $(".editParkingTicket").click(function(){
           var booking_id = $(this).data('id');
              $.ajax({
                  /* the route pointing to the post function */
                  url: '{{ url('day/edit-parking-ticket')}}/'+booking_id,
                  type: 'GET',
                  ...
                  success: function (data) {
                    ...
                    console.log(data)
                      ...
                      data.vehicle_num ? $("#vehicle").val(data.vehicle_num) : $("#vehicle").val("N/A");
...
function blocked_vehicle(booking_id){
  $("#blocked_booking_id").val(booking_id);  
  $("#blockedModal").modal('show');	
}
function unblocked_vehicle(booking_id){
    $("#unblocked_booking_id").val(booking_id);  
    $("#unblockedModal").modal('show');	
}
</script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootbox.js/4.4.0/bootbox.min.js"></script>
@endpush