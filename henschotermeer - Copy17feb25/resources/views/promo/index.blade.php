                            ...
                            <table id="listingDataTable" class="table table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>@lang('promo.valid_group_no')</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$promo->id}}</td>
                                        <td>{{$promo->group_name}}</td>
                                        <td>{{$promo->promo_number_limit}}</td>
                                        <td>{{$promo->code ? $promo->code : 'N/A'}}</td>
                                        <td data-sort="{{$promo->start_date}}">
                                       {{ date('d-m-Y',strtotime($promo->start_date)) }}
                                        </td>
                                        {{-- <td>
                                            @if(!empty($promo->bookings) && $promo->bookings !==null)
                                                @foreach($promo->bookings as $booking)
                                                    {{$booking->vehicle_num}} @if(!$loop->last) , @endif
                                                @endforeach
                                            @endif
                                        </td> --}}
                                        ...
                                        <td>
													<input type="hidden" name="copy_text"
                                                        id="promo-{{ base64_encode($promo->live_id) }}"
                                                        value="{{ env('API_BASE_URL') . '/' . 'register/email/location/group/promo/' . base64_encode($promo->live_id) . '/' . base64_encode($location->live_id) }}">
                                                    <a title="@lang('promo.copy')" class="copy_link btn btn-success btn-sm"
                                                        data-id="{{ base64_encode($promo->live_id) }}"
											...
											<a 
                                                href="{{url('promo/edit/'.$promo->id)}}"
                                                class="btn btn-primary btn-sm"  
                                                style="cursor:pointer;">
                                                <i class="fa fa-edit" ></i>
                                            </a>
											 <a
                                            href="{{url('group/viewgroupinfo/'.$promo->id)}}"
                                            title="@lang('promo.view')"
                                            class="view btn btn-success btn-sm mt-5"
                                            data-id="{{$promo->id}}"
                                            style="cursor:pointer;"
