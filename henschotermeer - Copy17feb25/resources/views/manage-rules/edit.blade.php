...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="white-box">
                    <h3 class="box-title pull-left">@lang('access-rules.edit_rule')</h3>
                    ...
                    <form 
                        id="parkingRule" 
                        action="{{url('manage-rules/update')}}" 
                        method="POST" 
                        ...
                        <div id="rootwizard">
                            ...
                            <input type="hidden" name="rule_slug" id="rule_slug" value="{{ $rule->slug }}">
                            <div class="tab-content">
                                <div class="tab-pane active" id="tab1">
                                    ...
                                    <div class="form-group {{ $errors->first('status', 'has-error') }}">
                                        ...
                                        <div class="col-sm-10">
                                            <select name="status" id="status" class="form-control">
                                                ...
                                                <option value="1" @if(isset($rule->access->enable) && $rule->access->enable) selected @endif>@lang('access-rules.status_enable')</option>
                                                <option value="0" @if(isset($rule->access->enable) && !$rule->access->enable) selected @endif>@lang('access-rules.status_disable')</option>
                                            </select>
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" title="@lang('devices.device_directions')" name="device_direction">
                                                ...
                                                <option value="bi-directional"
                                                    @if (old('device_direction',isset($rule->access->device_direction) && $rule->access->device_direction ) == 'bi-directional') selected="selected" @endif>
                                                    @lang('devices.bi_directional')
                                                </option>
                                                <option value="in"
                                                    @if (old('device_direction',isset($rule->access->device_direction) && $rule->access->device_direction ) == 'in') selected="selected" @endif>
                                                    @lang('devices.in')
                                                </option>
                                                <option value="out"
                                                    @if (old('device_direction',isset($rule->access->device_direction) && $rule->access->device_direction ) == 'out') selected="selected" @endif>
                                                    @lang('devices.out')
                                                </option>
                                            </select>
                                        </div>
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" title="@lang('devices.barcode_type')" name="barcode_type">
                                                ...
                                                <option value="parking_barcode"
                                                    @if (old('barcode_type',isset($rule->access->barcode_type) && $rule->access->barcode_type ) == 'parking_barcode') selected="selected" @endif>
                                                    @lang('access-rules.parking_barcode')
                                                </option>
                                                <option value="person_barcode"
                                                    @if (old('barcode_type',isset($rule->access->barcode_type) && $rule->access->barcode_type ) == 'person_barcode') selected="selected" @endif>
                                                    @lang('access-rules.person_barcode')
                                                </option>
                                                <option value="pos_barcode"
                                                    @if (old('barcode_type',isset($rule->access->barcode_type) && $rule->access->barcode_type ) == 'pos_barcode') selected="selected" @endif>
                                    ...
                                    <div
                                        class="form-group match_mode {{ $errors->first('plate_match_mode', 'has-error') }}">
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" title="@lang('devices.barcode_type')" name="plate_match_mode">
                                                ...
                                                {{-- <option value="disable"
                                                    @if (old('plate_match_mode',$rule->access->plate_match_mode) == 'disable') selected="selected" @endif>
                                                    @lang('access-rules.disable')
                                                </option> --}}
                                                <option value="low"
                                                    @if (old('plate_match_mode',isset($rule->access->plate_match_mode) && $rule->access->plate_match_mode ) == 'low') selected="selected" @endif>
                                                    @lang('access-rules.low')
                                                </option>
                                                <option value="medium"
                                                    @if (old('plate_match_mode',isset($rule->access->plate_match_mode) && $rule->access->plate_match_mode ) == 'medium') selected="selected" @endif>
                                                    @lang('access-rules.medium')
                                                </option>
                                                <option value="high"
                                                    @if (old('plate_match_mode',isset($rule->access->plate_match_mode) && $rule->access->plate_match_mode ) == 'high') selected="selected" @endif>
        ...
        <div id="pre_post_booking" class="modal fade">
            <div class="modal-dialog modal-confirm">
                <div class="modal-content">
                        ...
                        <div class="modal-footer justify-content-center">
                            ...
                            <button type="submit" class="btn btn-danger"  data-dismiss="modal">@lang('regression.ok')</button>
                        </div>
                </div>
            </div>
        </div>
